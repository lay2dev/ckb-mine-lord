    ...
    <q-pull-to-refresh @refresh="refresh">
      <div class="q-py-sm">
        <q-banner
          ...
          rounded
          dense
          :class="autoRefresh?'bg-blue-grey-8':'bg-blue-grey-10'"
          v-touch-hold:700.mouse="autoRefreshSwitch">
          <template v-slot:avatar>
            ...
            <small
              class="text-warning text-bold vertical-middle q-mx-xs"
            >[ {{ epochProgress.progress }} / {{ epochProgress.length }} ]</small>
          </template>
          <q-linear-progress
            class="q-px-md"
            dark
        ...
        <div class="row q-mt-sm justify-between rounded-borders q-pa-sm bg-blue-grey-10 text-white">
          ...
          <div>
            <span class="stat-label">TBLK: </span>
            <span class="stat-value">{{ height }}</span>
          </div>
          <div>
            <span class="stat-label">DFCT: </span>
            <span class="stat-value">{{ numberWithCommas(parseInt(difficulty / 1000)) }} T</span>
          </div>
          <div>
            <span class="stat-label">HASH: </span>
            <span class="stat-value">{{ numberWithCommas(parseInt(hashrate / 1000)) }} G</span>
          </div>
        </div>
      </div>
        ...
        <q-tab-panel name="rank">
          <q-list dark bordered separator>
            ...
            >
              ...
              <q-item-section>
                <q-item-label class="text-primary nick">{{getNick(item.addr)}}</q-item-label>
                <q-item-label
                  caption
                  class="addr"
                >{{item.addr.slice(0, 3) + '...' + item.addr.slice(-6)}}</q-item-label>
              </q-item-section>
              <q-item-section side>
                <q-item-label class="balance">
                  {{item.amount.slice(0, -8)}}
                  <small class="text-grey-7">CKB-TEST</small>
                </q-item-label>
                <q-item-label class="exp">
                  {{item.exp.split('.')[0]}}
                  <small class="text-grey-5">CKB-REAL</small>
                </q-item-label>
              </q-item-section>
            </q-item>
          </q-list>
        ...
        <q-tab-panel name="luck" class="bg-deep-blue">
          <q-list dark bordered separator>
            ...
            >
              ...
              <q-item-section>
                <q-item-label class="text-amber nick">{{getNick(item.lucky)}}</q-item-label>
                <q-item-label
                  caption
                  class="addr"
                >{{item.lucky.slice(0, 3) + '...' + item.lucky.slice(-6)}}</q-item-label>
              </q-item-section>
              <q-item-section side>
                <q-item-label class="row justify-between" style="min-width: 85px">
                  <span>height:</span>
                  <span class="text-primary text-bold">{{item.startNumber}}</span>
                </q-item-label>
              </q-item-section>
            </q-item>
          </q-list>
...
const nervos = 'https://api.explorer.nervos.org/api/v1/statistics'
export default {
  name: 'Index',
  data () {
    return {
  ...
  methods: {
    ...
    load: async function () {
      try {
        ...
        console.log('res', res)
        const {
          data: { total, list, epochs, epoch, height }
        } = res[0]
        this.total = total
        this.list = list
        this.epoch = epoch
        ...
        this.difficulty = ~~(difficulty / (1000 ** 3))
        this.hashrate = ~~(parseInt(hashrate) / 1000)
    ...
    getNick: function (address) {
      let nick = address.slice(-3)
      switch (nick) {
        case 'yew': nick = 'uupool'; break
        case 'j6s': nick = 'SparkPool'; break
        case 'ugu': nick = 'f2pool'; break
        default : break
      }

      return nick
    },
    autoRefreshSwitch: function ({ evt, ...info }) {
      this.autoRefresh = !this.autoRefresh
      this.$q.notify({
        position: 'top',
        textColor: 'primary',
        timeout: 1000,
        message: `Auto Refresh ${this.autoRefresh ? 'ON' : 'OFF'}`
      })
